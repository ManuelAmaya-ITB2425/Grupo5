...
for row in raiz.findall('row'):
    ...
    if dataIncidencia is not None:
        try:
            ...
            contadorMalas += 1
            salidaMostrar.append(f"Error de formato de fecha: {dataIncidencia} para {nomCognom}")
            continue
    ...
    salidaMostrar.append(f'Marca_de_Temps:\033[0;31;40m'f'{marcaTemps}\033[0;0m   '
...
if contadorTotal > 0:
    ...
    salidaMostrar.append('\033[1;3;4;35m'f"\nTotal de incidencias procesadas:\033[0;35m {contadorTotal}")
    salidaMostrar.append('\033[1;3;4;32m'f"Porcentage de incidencias validas:\033[0;32m {porcentageBuenas:.2f}%")
    salidaMostrar.append('\033[1;3;4;31m'f"Porcentages de incidencias invalidas:\033[0;31m {porcentageMalas:.2f}%")
else:
    salidaMostrar.append("No se han procesado incidencias")

ficheroJSON = "incidencias.json"
with open(ficheroJSON, 'w', encoding='utf-8') as json_file:
    json.dump(salidaMostrar, json_file, ensure_ascii=False, indent=4)

print(f"\nEl fichero JSON '{ficheroJSON}' se ha creado exitosamente en la misma ubicacion que el script")